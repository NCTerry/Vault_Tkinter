...
def menu():
    ...
    # ==============================
    menuLabel1.set( "-----------------")
    menuLabel2.set( "1) Encrypt")
    menuLabel3.set( "2) Decrypt")
    menuLabel4.set( "3) Exit")
    menuLabel5.set( "-----------------")
    # ====================================================================================
    ...
    menuLabel3x.grid(row=3, column=0, columnspan=1, sticky=W)
    # -----------
    ...
    menuLabel5x.grid(row=5, column=0, columnspan=1, sticky=W)
    # ====================================================================================
    ...
    locationVARx.grid(row=12, column=0, columnspan=1, sticky=W)
    # =========
    usernameVARx = Label(self, textvariable=usernameVAR, height=1)  # Create a label object
    usernameVARx.grid(row=13, column=0, columnspan=1, sticky=W)
    # =========
    passwordVARx = Label(self, textvariable=passwordVAR, height=1)  # Create a label object
    passwordVARx.grid(row=14, column=0, columnspan=1, sticky=W)
    # =========
    # ===========================================================================
    # Variable to hold the user choice for the main menu.
    userIDVAR_input = StringVar(0)
    locationVAR_input = StringVar(None)
    usernameVAR_input = StringVar(None)
    passwordVAR_input = StringVar(None)
    # ===========================================================================
    # ------------------------
    userIDVAR_inputx = Entry(self, textvariable=userIDVAR_input, width=20)
    userIDVAR_inputx.grid(row=11, column=1, sticky=W)
    # ------------------------
    locationVAR_inputx = Entry(self, textvariable=locationVAR_input, width=20)
    locationVAR_inputx.grid(row=12, column=1, sticky=W)
    # ------------------------
    usernameVAR_inputx = Entry(self, textvariable=usernameVAR_input, width=20)
    usernameVAR_inputx.grid(row=13, column=1, sticky=W)
    # ------------------------
    passwordVAR_inputx = Entry(self, textvariable=passwordVAR_input, width=20)
    passwordVAR_inputx.grid(row=14, column=1, sticky=W)
    # ====================================================================================

    button1 = Button(self, text="Enter Information", width=15, command=main_menu_button())
...
def openFiles( selection): # There are two selections in our dropdown box. If they select "Student Grades" then we will call in the openGrades function, or else it must be to call in the rank instead.
...
def decrypt_in_Main():
    ...
    for x in range(len(password_List)):
        ...
        glo_2Dkey_Int, glo_UID_Str, glo_U_P_Str, glo_Username_Str, glo_location_Str = now_decrypt_Password(key,
...
# Create the app it add the app title, create the window size.
self = Tk()
self.title('Password Vault')
self.geometry('800x500+1+1')

final_list = []
# -----------
# This will be part of the dropdown box.
studFiles = StringVar()  # Files will contain a string variable
studFiles.set(None)  # The files will be set empty by default.
files = ['Encrypt', 'Decrypt', 'Exit']  # List with The 2 options that we want in the drop down box.
studDropDown = OptionMenu( studFiles, *files, command=openFiles).grid(row=10)

# to hold the user selection from the menu
user_menu_choice = StringVar()
self.menu()
mainloop()
# -----------